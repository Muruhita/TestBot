...
export default function HighRankReportForm() {
  ...
  const [formData, setFormData] = useState({ rankRange: '', workLink: '' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await fetch('/api/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'highrank', fullName: nickname, ...formData }) });
      ...
      <div className="form-container">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <textarea value={formData.workLink} onChange={(e) => setFormData({...formData, workLink: e.target.value})} required rows="4" />
          </div>
